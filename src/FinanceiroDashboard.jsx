...
// Regressão linear simples (mínimos quadrados) sobre um array de valores y,
// assumindo x = 0,1,2... Retorna {slope, intercept} para projeção.
...
export default function FinanceiroDashboard({ onVoltar }) {
  ...
  async function carregar() {
    ...
    if (fluxoRes.error) {
      setErro(`Erro ao carregar fluxo de caixa: ${fluxoRes.error.message}`);
      setCarregando(false);
      return;
    }
    if (saldoRes.error) {
      setErro(`Erro ao carregar saldo: ${saldoRes.error.message}`);
      setCarregando(false);
      return;
    }
  ...
  // Monta os dados do gráfico: histórico real + meses futuros projetados
  const dadosGrafico = useMemo(() => {
    ...
    // Usa só os últimos N meses pra calcular a tendência (evita distorção
    // de um histórico muito longo com padrões antigos)
    ...
    // Ponte: repete o último ponto real também como "previsto" pra linha
    // pontilhada conectar visualmente sem quebrar
          ...
          <div className="relative flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-100">Fluxo de caixa e previsão</p>
              </div>
            </div>
          </div>
        </header>
